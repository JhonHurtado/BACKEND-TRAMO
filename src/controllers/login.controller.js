...
export const createAdmin = async (req, res)=>{
    try {
        ...
        if(usuario && passHaas){
            connection.query(`INSERT INTO Tbl_Administradores SET ?`, {Usuario:usuario, Contrasena:passHaas}, async(error, results)=>{
                if(error){
                    console.log(error)
                }else{
    ...
    try {
        ...
        if(usuarioAdmin && adminContra){
            connection.query(`SELECT * FROM tbl_administradores WHERE Usuario=?`, [usuarioAdmin], async (error , results)=>{
                if(results.length == 0 || !(await bcryptjs.compare(adminContra, results[0].Contrasena))){
